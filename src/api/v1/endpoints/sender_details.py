...
async def create_sender_detail(
    data: SenderDetailCreate, current_user=Depends(jwt_handler.get_current_user)
):
    """Create a new sender detail"""
    return await sender_detail_service.create_sender_detail(
        user_id=current_user["user_id"], data=data.dict()
    )


@router.get("", response_model=SenderDetailsListResponse)
async def get_sender_details(current_user=Depends(jwt_handler.get_current_user)):
...
async def get_default_sender_detail(current_user=Depends(jwt_handler.get_current_user)):
...
async def delete_sender_detail(
    id: UUID, current_user=Depends(jwt_handler.get_current_user)
):
    """Delete a specific sender detail"""
    return await sender_detail_service.delete_sender_detail(
        id=id, user_id=current_user["user_id"]
    )
